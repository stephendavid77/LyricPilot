...
from .timecode_generator import load_timecode_json, TimecodeData, TimecodeEntry
...
async def start_song_playback(song_id: str, db: Session = Depends(get_db)):
    song = get_song(db, song_id)
    if not song or not song.processed or not song.timecode_path:
        raise HTTPException(status_code=404, detail="Song not found or not processed")
    ...
    return {"message": f"Playback started for {song.title}"}
